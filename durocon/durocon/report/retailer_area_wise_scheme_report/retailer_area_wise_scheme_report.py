...
def execute(filters=None):
	...
	columns = [
		...
		{"label": "Price", "fieldname": "price", "fieldtype": "Select", "options": "Gift, Amount","width": 100},
	...
	# Step 1: Get Schemes matching date range and Area-based type
	scheme_filters = {
		"scheme_type_for": "Area",
		"effective_from": ["<=", to_date],
		"effective_to": [">=", from_date]
	}
	scheme_list = frappe.get_list("Scheme", filters=scheme_filters, pluck='name')

	# Step 2: Get all Retailer customers with Area info
	...
	# Step 3: Match schemes with customers based on Area
	for scheme_name in scheme_list:
		sch_doc = frappe.get_doc("Scheme", scheme_name)

		# Determine applicable areas
		if sch_doc.applicable_for_all_areas:
			applicable_areas = [cust["territory"] for cust in customers]
		else:
			applicable_areas = [area.territory for area in sch_doc.applicable_for_areas]

		# Step 4: For each matching customer, check achievement
		for cust in customers:
			if cust["territory"] in applicable_areas:
				if area_filter and cust["territory"] != area_filter:
					continue
				...
				so_total = frappe.db.sql("""
					...
					""",
					(cust["name"], sch_doc.effective_from, sch_doc.effective_to)
				)[0][0] or 0.0
